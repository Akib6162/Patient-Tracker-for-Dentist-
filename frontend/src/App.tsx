...
export default function App() {
  ...
  useEffect(() => {
    ...
    function connect() {
      const token = localStorage.getItem("accessToken");
      if (!token) {
        // Not logged in yet — check again in 2s
      ...
      eventSource.onerror = () => {
        // Connection lost — close and retry in 3s
        eventSource?.close();
        eventSource = null;
                  ...
                  <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-8 text-center space-y-5">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
                        This application is assistant only. If you believe this is an error, please contact your system administrator.
                      </p>
                    </div>
                    <button
                      onClick={() => {
